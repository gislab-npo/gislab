...
from utils import to_decimal_array
...
class ProjectPage(PublishPage):
	...
	def validate(self):
		...
		base_layers = [layer for layer in self.plugin.layers_list() if self.plugin.is_base_layer_for_publish(layer)]
	...
	def get_metadata(self):
		...
		def base_layers_data(node):
			if node.children:
				...
				if layer_resolutions:
					layer_resolutions = publish_resolutions(self.plugin.wmsc_layer_resolutions(layer))
					...
					if layer.hasScaleBasedVisibility():
						layer_visible_resolutions = self.plugin.filter_visible_resolutions(layer_data['resolutions'], layer)
		...
		overlays_order = [layer.id() for layer in self.plugin.layers_list() if self.plugin.is_overlay_layer_for_publish(layer)]
