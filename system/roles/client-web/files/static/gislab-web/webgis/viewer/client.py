...
class WebgisClient(object):
	...
	def project_request(self, request):
		...
		if context.get('base_layers'):
			# first level base layers is enough to check
			base_layers_names = [layer.get('name', '') for layer in context['base_layers']]
			for google_layer_name in GOOGLE_LAYERS.iterkeys():
				if google_layer_name in base_layers_names:
					google = True
					break
	...
	def vector_layers_request(self, request):
		...
		if project:
			...
			drawing_filename = os.path.join(settings.GISLAB_WEB_PROJECT_ROOT, '{0}.geojson'.format(ows_project))
